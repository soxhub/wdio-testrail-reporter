...
`;
    ...
    /**
     * @param {{title, logs, duration}} test
     * @return {string}
     */
    getRunComment(test) {
        let comment = test.title;
        if (WdioTestRailReporter.logs && WdioTestRailReporter.logs.length) {
            comment += `\n${WdioTestRailReporter.logs.join("\n")}`;
            WdioTestRailReporter.logs = []; //flush logs
        }
        return comment;
    }

    static addLog(message) {
        WdioTestRailReporter.logs.push(message);
    }
}

// logs can be used to extend comment of each run
WdioTestRailReporter.logs = [];
