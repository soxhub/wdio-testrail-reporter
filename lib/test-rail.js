...
class TestRail {
     ...
     * @return {*}
     */
    addRun(name, description, results) {
        let cases = [];
        let payload = {
            "suite_id": this.options.suiteId,
            "name": name,
        ...
        if (this.options.includeAll === false) {
            ...
            payload["case_ids"] = cases;
        }
        return this._post(`add_run/${this.options.projectId}`, payload);
    }
    ...
    publish(name, description, results, callback = undefined) {
        let run = this.addRun(name, description, results);
